...
class MySQL:
    def __init__(self):
        try:
            ...
            self.db = MySQLdb.connect('localhost', 'root', 'chensy123', 'Auction_Spider_GPai', 3306)
            self.cur = self.db.cursor()
            print(DateTimeUtil.get_current_time(), "Connection DB Successful...")
        except MySQLdb.Error as e:
            print(DateTimeUtil.get_current_time(), "Connection DB Failure: %d: %s" % (e.args[0], e.args[1]))
    ...
    def get_statuses(self):
        sql_select = "SELECT Id, StatusId, StatusName, IsSpiderThisStatus FROM Auction_Statuses"
        return self.select(sql_select)
    ...
    def upsert_auction(self, auction_json):
        insert_check = 'SELECT COUNT(*) FROM Auctions WHERE AuctionId = ' + auction_json['AuctionId']
        insert_sql = "INSERT INTO %s (%s) VALUES (%s)" % ('Auctions',
